...
//   }
//
// The macro ends with a `abesim::StatusBuilder` which allows the returned
...
// to avoid confusion between a `abesim::StatusBuilder` and a
...
#define PANDEMIC_RETURN_IF_ERROR(expr)                              \
  ...
  if (::abesim::status_macro_internal::StatusAdaptorForMacros       \
          status_macro_internal_adaptor = {(expr), PANDEMIC_LOC}) { \
  } else /* NOLINT */                                               \
    return status_macro_internal_adaptor.Consume()

// Executes an expression `rexpr` that returns a `abesim::StatusOr<T>`. On
...
// well as a `abesim::StatusBuilder` object populated with the error and
...
#define PANDEMIC_STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_(statusor, lhs, rexpr, \
  ...
  if (ABSL_PREDICT_FALSE(!statusor.ok())) {                                 \
    ::abesim::StatusBuilder _(std::move(statusor).status(), PANDEMIC_LOC);  \
...
class StatusAdaptorForMacros {
 public:
  ...
  abesim::StatusBuilder&& Consume() { return std::move(builder_); }

 private:
  abesim::StatusBuilder builder_;
};
