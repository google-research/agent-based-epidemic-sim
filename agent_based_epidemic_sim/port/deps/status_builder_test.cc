...
const SourceLocation kLoc = PANDEMIC_LOC;
...
// copybara:strip_begin(simple implementation of status blows this up)
TEST(StatusBuilderTest, DISABLED_Size) {
...
TEST(StatusBuilderTest, Ctors) {
  EXPECT_EQ(ToStatus(StatusBuilder(kZomg, PANDEMIC_LOC) << "zomg"),
            absl::Status(kZomg, "zomg"));
}

TEST(StatusBuilderTest, Identity) {
  SourceLocation loc(PANDEMIC_LOC);
...
TEST(CanonicalErrorsTest, CreateAndClassify) {
  for (const auto& test : kCanonicalErrorTests) {
    ...
    absl::Status status = test.creator(PANDEMIC_LOC) << message;
